...
<div class="container-xxl">
    <div class="title h1 text-center mt-3">
        <fmt:message bundle="${loc}" key="local.technical-tasks.header"/>
    </div>
            ...
            <c:if test="${ role == 'CUSTOMER' }">
                <a href="?command=to-create-technical-task">
                    <button id="create-task" type="button" class="btn btn-primary">
                        <fmt:message bundle="${loc}" key="local.technical-tasks.new"/>
                    </button>
                </a>
            </c:if>
        </div>
        <form id="radio">
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio"
                       name="unitPerPage" id="option1" value="4" checked>
                <label class="form-check-label" for="option1">4</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio"
                       name="unitPerPage" id="option2" value="8">
    ...
    <div class="frame row justify-content-center">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <c:choose>
                                <c:when test="${task.status == 'WAIT_PROJECT'}">
                                    <div class="badge"><span class="violet">
                                        <fmt:message bundle="${loc}"
                                                     key="local.technical-tasks.wait-project"/>
                                    </span>
                                    </div>
                                </c:when>
                                <c:when test="${task.status == 'ON_PROJECT'}">
                                    <div class="badge"><span class="blue">
                                        <fmt:message bundle="${loc}"
                                                     key="local.technical-tasks.on-project"/>
                                    </span>
                                    </div>
                                </c:when>
                                <c:when test="${task.status == 'COMPLETED'}">
                                    <div class="badge"><span class="green">
                                        <fmt:message bundle="${loc}"
                                                     key="local.technical-tasks.complited"/>
                                    </span>
                                    </div>
                                </c:when>
                            </c:choose>
                        ...
                        <div class="requirements mt-5 mx-3">
                            ...
                            <div style="display:none;" class="list">
                                <table class="table my-5">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col">
                                            <fmt:message bundle="${loc}"
                                                         key="local.requirement.experience"/>
                                        </th>
                                        <th scope="col">
                                            <fmt:message bundle="${loc}"
                                                         key="local.requirement.salary"/>
                                        </th>
                                        <th scope="col">
                                            <fmt:message bundle="${loc}"
                                                         key="local.requirement.qualification"/>
                                        </th>
                                        <th scope="col">
                                            <fmt:message bundle="${loc}"
                                                         key="local.requirement.primary"/>
                                        </th>
                                        <th scope="col">
                                            <fmt:message bundle="${loc}"
                                                         key="local.requirement.comment"/>
                                        </th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                ...
                                <div class="d-flex justify-content-between mr-3">
                                    <div class="d-flex flex-row align-items-center">
                                        <button type="button" class="details btn btn-primary">
                                            <fmt:message bundle="${loc}"
                                                         key="local.technical-tasks.details"/>
                                        </button>
                                    ...
                                    <form action="controller">
                                        ...
                                        <c:if test="${ role == 'MANAGER' }">
                                            ...
                                            <button type="submit" class="btn btn-primary">
                                                <fmt:message bundle="${loc}"
                                                             key="local.technical-tasks.create"/>
                                            </button>
                                        </c:if>
                                        <c:if test="${ role == 'CUSTOMER' }">
                                            <input type="hidden" class="command" name="command"
                            ...
                            <div class="mt-3 d-flex justify-content-between mr-3">
                                <div class="d-flex flex-row align-items-center">
                                        <span class="text1">
                                            <fmt:message bundle="${loc}"
                                                         key="local.technical-tasks.deadline"/>
                                             <span class="text2">
        ...
        <div class="pagging row" aria-label="...">
            ...
            <a class="next col" style="text-align: center">
                <fmt:message bundle="${loc}" key="local.pagination.next"/>
            </a>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">
                        <fmt:message bundle="${loc}" key="local.technical-tasks.update-header"/>
                    </h5>
                </div>
                <form action="controller" method="post"
                      onSubmit="return validateTaskForm()" novalidate>
                    <input type="hidden" name="command" value="update-technical-task"/>
                    <input type="hidden" name="technical-task-id" id="updateTaskId"/>
                    <div class="modal-body">
                        <label for="name" class="col-form-label">
                            <fmt:message bundle="${loc}" key="local.technical-tasks.update-name"/>
                        </label>
                        ...
                        <label for="name" class="col-form-label">
                            <fmt:message bundle="${loc}" key="local.technical-tasks.update-overview"/>
                        </label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">
                            <fmt:message bundle="${loc}" key="local.technical-tasks.update-cancel"/>
                        </button>
                        <button type="submit" class="btn btn-primary" id="create-task-button">
                            <fmt:message bundle="${loc}" key="local.technical-tasks.update-submit"/>
                        </button>
                    </div>
                </form>
            </div>
