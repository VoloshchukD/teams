...
<div class="container-xxl">
    ...
    <div class="pagging-frame row justify-content-center">
        <div class="elements row card-deck justify-content-center mt-3">
            <c:forEach items="${bills}" var="bill">
                <div class="element col-3 card text-center m-2 ">
                    <div class="card-header">
                        <h4 class="my-0 font-weight-normal"><fmt:message bundle="${loc}" key="local.bills.number"/>${bill.id}</h4>
                    </div>
                    <div class="card-body text-center">
                        <h1 class="card-title pricing-card-title">$${bill.amountDue} <small class="text-muted">/</small>
                            <c:choose>
                                <c:when test="${bill.status == 'NOT_PAID'}">
                                    <div class="badge"><span class="red"><fmt:message bundle="${loc}" key="local.bills.not-paid"/></span>
                                    </div>
                                </c:when>
                                <c:when test="${bill.status == 'PAID'}">
                                    <div class="badge"><span class="green"><fmt:message bundle="${loc}" key="local.bills.paid"/></span>
                                    </div>
                                </c:when>
                            </c:choose>
                        </h1>
                        <h6 class="my-0 font-weight-normal"><fmt:message bundle="${loc}" key="local.bills.project"/>: lkdsmfsdf</h6>
                        <hr>
