...
<div class="container-xxl">
    <div class="title h1 text-center mt-3">
        <fmt:message bundle="${loc}" key="local.projects.header"/>
    </div>
        ...
        <div class="row justify-content-center">
            <div class="col-6">
                <div class="alert alert-success alert-dismissible" role="alert" id="success">
                    <strong>
                        <fmt:message bundle="${loc}" key="local.message.success.header"/>
                    </strong>
                    <fmt:message bundle="${loc}" key="local.message.project.success"/>
                    <fmt:message bundle="${loc}" key="local.message.project.created-name"/>
                    <c:out value="${recentlyCreatedProjectName}"/>
                                ...
                                <c:when test="${project.state == 'IN_PROGRESS'}">
                                    <div class="badge"><span class="green">
                                        <fmt:message bundle="${loc}"
                                                     key="local.projects.status-in-progress"/>
                                    </span>
                                    </div>
                                </c:when>
                                <c:when test="${project.state == 'FINISHED'}">
                                    <div class="badge"><span class="blue">
                                        <fmt:message bundle="${loc}" key="local.projects.status-finished"/>
                                    </span>
                                    </div>
                                </c:when>
                            </c:choose>
                                    ...
                                    <c:if test="${ project.state != 'FINISHED' }">
                                        ...
                                        <c:if test="${ role == 'MANAGER' }">
                                            <a href="?command=to-bill-creation&project-id=${project.id}">
                                                <button type="submit" class="btn btn-primary">
                                                    <fmt:message bundle="${loc}"
                                                                 key="local.projects.add-bill"/>
                                                </button>
                                            </a>
                                            <a href="?command=to-seek-employees&project-id=${project.id}">
                                                <button type="submit" class="btn btn-primary">
                                                    <fmt:message bundle="${loc}"
                                                                 key="local.projects.add-employees"/>
                                                </button>
                                            </a>
                                        </c:if>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="controller" method="post"
                    ...
                    <input type="hidden" name="command" value="update-project"/>
                    <input type="hidden" name="project-id" id="updateProjectId"/>
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="name" class="col-form-label">
                                <fmt:message bundle="${loc}" key="local.projects.update-name"/>
                            </label>
                        ...
                        <div class="form-group">
                            <label for="description" class="col-form-label">
                                <fmt:message bundle="${loc}" key="local.projects.update-description"/>
                            </label>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">
                            <fmt:message bundle="${loc}" key="local.projects.update-cancel"/>
                        </button>
